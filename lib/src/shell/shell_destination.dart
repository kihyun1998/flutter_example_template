...
///
/// One of these was once named after its only member, which is a set named
/// after an element. The second member is what made that visible.
enum ShellCategory {
  recipes('Recipes'),
  scenarios('Scenarios'),
  pages('Pages');
...
class StageDestination extends ShellDestination {
  ...
  /// displayed cannot disagree. Where a consumer holds a directory of
  /// self-contained files to an import allow-list, offering this pane over a
  /// file that rule excludes changes what the pane means — so a destination
  /// outside that zone passes null even when it *is* one file.
  final String? source;
}
