...
const _spec = [
  ...
  SettingGroup(
    id: 'other',
    title: 'Other',
    features: [SettingFeature(id: 'glow', title: 'Glow', switchId: 'glowOn')],
  ),
];
...
void main() {
  group('the panes run on a host that knows nothing about this app', () {
    ...
    testWidgets('a dot writes back through the port, not through a settings '
        'object', (t) async {
    ...
    testWidgets(
      'a host with no extras gets none, and says nothing to get them',
      (t) async {
    ...
    testWidgets('the preset bar draws the host\'s presets and their guidance', (
      ...
      await t.pumpWidget(
        MaterialApp(
          home: Scaffold(body: PresetBar(host: host)),
        ),
      );
    ...
    testWidgets(
      ...
      (t) async {
        ...
        await t.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: PresetBar(host: _FakeHost(withPresets: false)),
            ),
          ),
        );
